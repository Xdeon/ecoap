-module(benchmark).
-export([coap_discover/2, coap_get/5, coap_post/4, coap_put/4, coap_delete/4, coap_observe/5, coap_unobserve/2, handle_info/2, coap_ack/2]).
...
coap_get(_EpID, [<<"benchmark">>], _Name, _Query, _Options) ->
    #coap_content{payload = <<"hello world">>};
coap_get(_EpID, [<<"fibonacci">>], _Name, [], _Options) ->
    #coap_content{payload = <<"fibonacci(20) = ", (integer_to_binary(fib(20)))/binary>>};
coap_get(_EpID, [<<"fibonacci">>], _Name, [Query|_], _Options) ->
...
coap_get(_EpID, [<<"helloWorld">>], _Name, _Query, _Options) ->
    #coap_content{payload = <<"Hello World!">>, format = 0};
coap_get(_EpID, [<<"shutdown">>], _Name, _Query, _Options) ->
    #coap_content{payload = <<"Send a POST request to this resource to shutdown the server">>};
coap_get(_EpID, _Prefix, _Name, _Query, _Options) ->
    {error, 'NotFound'}.
...
coap_delete(_EpID, _Prefix, _Name, _Options) ->
    {error, 'MethodNotAllowed'}.

coap_observe(_EpID, _Prefix, _Name, _Ack, _Options) ->
    {error, 'MethodNotAllowed'}.

coap_unobserve(_Obstate, _Options) ->
    ok.

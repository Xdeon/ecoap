...
handle_call({start_endpoint, EpID, {Method, OptionList, Content}}, From, State = #state{sock_pid = SockPid, request_refs = Refs}) ->
	...
	% io:format("initial req: ~p~n", [{Method, OptionList, Content}]),
	{noreply, State#state{from = From, endpoint_pid = EndpointPid, request_refs = maps:put(Ref, {Method, OptionList, Content, <<>>}, Refs)}};
...
handle_cast({start_endpoint, EpID, {Method, OptionList, Content}, ClientRef}, State = #state{sock_pid = SockPid, request_refs = Refs}) ->
	{ok, EndpointPid} = ecoap_socket:get_endpoint(SockPid, EpID),
	{ok, Ref} = request_block(EndpointPid, Method, OptionList, Content),
	{noreply, State#state{client_ref = ClientRef, endpoint_pid = EndpointPid, request_refs = maps:put(Ref, {Method, OptionList, Content, <<>>}, Refs)}};
...
handle_info({coap_response, _EpID, EndpointPid, Ref, #coap_message{code={ok, continue}, options=Options}}, 
	State = #state{request_refs = Refs, endpoint_pid = EndpointPid}) ->
	case find_ref(Ref, Refs) of
		undefined -> {noreply, State};
		Req = {Method, OptionList, Content, _} ->
...
handle_info({coap_response, _EpID, EndpointPid, Ref, Message=#coap_message{code={ok, Code}, options=Options, payload=Data}}, 
	State = #state{request_refs = Refs, client_pid = ClientPid, client_ref = ClientRef, endpoint_pid = EndpointPid, from = From}) ->
	case find_ref(Ref, Refs) of
		undefined -> {noreply, State};
		{Method, OptionList, Content, Fragment} ->
			case coap_message_utils:get_option('Block2', Options) of
		            {Num, true, Size} ->
		                ...
		                {noreply, State#state{request_refs = store_ref(Ref2, {Method, OptionList, Content, <<Fragment/binary, Data/binary>>}, delete_ref(Ref, Refs))}};
		            _Else ->
		                % not segmented
...
handle_info({coap_response, _EpID, EndpointPid, Ref, Message=#coap_message{code=Code}}, 
	State = #state{request_refs = Refs, client_pid = ClientPid, client_ref = ClientRef, endpoint_pid = EndpointPid, from = From}) ->
	case find_ref(Ref, Refs) of
		undefined -> {noreply, State};
		_ -> 
...
handle_info({coap_error, _EpID, EndpointPid, Ref, Error}, 
	State = #state{request_refs = Refs, client_pid = ClientPid, client_ref = ClientRef, endpoint_pid = EndpointPid, from = From}) ->
	case find_ref(Ref, Refs) of
		undefined -> {noreply, State};
		_ ->
...
terminate(_Reason, #state{sock_pid = SockPid, endpoint_pid = EndpointPid}) ->
	ok = coap_endpoint:close(EndpointPid),

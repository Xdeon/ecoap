...
server_simple_storage_test_() ->
    {setup,
        fun() ->
            ...
            ecoap_registry:register_handler([{[<<"storage">>], ?MODULE}]),
...
simple_storage_test(Client) ->
    ...
    ?_assertEqual({ok, 'Content', #coap_content{payload= <<"1">>, options=#{'ETag'=>[<<"1">>]}}},
        ecoap_client:request(Client, 'GET', "coap://127.0.0.1/storage/one")),

    ?_assertEqual({ok, 'Valid', #coap_content{options=#{'ETag'=>[<<"1">>]}}},
        ecoap_client:request(Client, 'GET', "coap://127.0.0.1/storage/one",
            <<>>, #{'ETag'=>[<<"1">>]})),
    ...
    ?_assertEqual({ok, 'Content', #coap_content{payload= <<"2">>, options=#{'ETag'=>[<<"2">>]}}},
        ecoap_client:request(Client, 'GET', "coap://127.0.0.1/storage/one")),
    ...
    ?_assertEqual({ok, 'Valid', #coap_content{options=#{'ETag'=>[<<"2">>]}}},
        ecoap_client:request(Client, 'GET', "coap://127.0.0.1/storage/one",
            <<>>, #{'ETag'=>[<<"1">>, <<"2">>]})),

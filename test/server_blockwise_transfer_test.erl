...
coap_post(_EpID, _Prefix, [], Request) ->
    {ok, 'Content', ecoap_content:get_content(Request)}.
...
blockwise_transfer(Client) ->
    ...
    ?_assertMatch({ok, {ok, 'Content'}, 
        #ecoap_content{payload= <<"</.well-known/core>,</reflect>,</text>">>, options=#{'Content-Format':= <<"application/link-format">>}}}, 
    ...
    ?_assertEqual({ok, {ok, 'Created'}, #ecoap_content{}}, 
        begin 
            #ecoap_content{payload=Payload, options=Options} = test_utils:text_resource(128),
            ecoap_client:put(Client, "/reflect", Payload, Options) 
        end),
    ?_assertEqual({ok, {ok, 'Created'}, #ecoap_content{}}, 
        begin
            #ecoap_content{payload=Payload, options=Options} = test_utils:text_resource(1024),
            ecoap_client:put(Client, "/reflect", Payload, Options)
        end),
    ?_assertEqual({ok, {ok, 'Created'}, #ecoap_content{}}, 
        begin
            #ecoap_content{payload=Payload, options=Options} =  test_utils:text_resource(1984),
            ecoap_client:put(Client, "/reflect", Payload, Options)
        end),
